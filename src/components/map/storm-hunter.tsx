  ...
  return (
    <>
      <motion.button
        ...
        className={cn(
          'fixed top-[calc(3.5rem+env(safe-area-inset-top)+0.5rem)] right-3 z-30 flex items-center gap-1.5 rounded-full border px-3 py-2 text-xs font-medium shadow-lg backdrop-blur-md transition-colors md:top-[5rem] lg:right-[26.5rem]',
          active
      ...
      <AnimatePresence>
        {active && (
          <motion.div
            ...
            exit={{ y: -20, opacity: 0 }}
            className="pointer-events-none fixed top-[calc(3.5rem+env(safe-area-inset-top)+3.5rem)] left-1/2 z-30 -translate-x-1/2 rounded-full border border-violet-400/40 bg-gradient-to-r from-indigo-600/85 to-violet-600/85 px-4 py-1.5 text-xs font-medium whitespace-nowrap text-white shadow-lg backdrop-blur-md md:top-[8rem]"
            role="status"
          >
